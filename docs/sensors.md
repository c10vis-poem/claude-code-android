...
Native Android sensor access from Termux using the NDK (Native Development Kit) `ASensorManager` API. No root, no JNI, no proot required.

> **Note:** This uses `ASensorManager` from the Android NDK, loaded via `dlsym` at runtime. It does not use the `termux-sensor` command (which relies on the Termux:API Java bridge). The NDK approach provides direct hardware access with higher sample rates and lower latency than the Java bridge (JNI) path.

---

## Summary

**9 of 11 standard sensor types confirmed working in my testing** on a Samsung S26 Ultra (Android 16, ARM64), using a compiled C binary in native Termux linked against `-landroid -ldl`.

---

## Hardware Inventory

45 sensors detected via ASensorManager on the test device (Samsung S26 Ultra, Android 16). Counts vary by device and enumeration API: [`docs/avf-guide.md`](avf-guide.md) (the Android Virtualization Framework guide in this repo) reports 42 sensors on Pixel 10 Pro via `dumpsys sensorservice`. Different devices, different APIs; both numbers are accurate for their respective contexts.
...
| 4 | Gyroscope | WORKS |
| 5 | Light (ALS, Ambient Light Sensor) | WORKS |
...
- **Light**: 1050-1341 lux (the SI unit for illuminance; bright indoor, fluctuating)
- **Pressure**: 1012.95 hPa (hectopascals, the standard meteorological unit; sea-level-equivalent indoor pressure)
...
1. **All standard sensors I tested worked from native Termux** using the ASensorManager NDK API loaded via `dlsym`. No JNI, no root, no proot needed.
...
The sensor reader supports vendor-specific sensor types (type IDs above 65536) via list-scan fallback when `getDefaultSensor` returns NULL. On my Samsung Galaxy S26 Ultra (Android 16), these Samsung-specific sensors were detected and resolved:
